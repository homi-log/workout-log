...
- 운동(`workout_db`), 체중(`workout_weight_db`), 식단(`workout_meal_db`) 세 가지 기록이
  각각 브라우저의 **localStorage**에 저장됩니다. 기기·브라우저별로 유지됩니다.
- **여러 기기 자동 동기화(선택):** 헤더의 **☁ 버튼**에 Google Apps Script 웹앱 URL을 등록하면
  본인 Google 시트에 세 기록이 함께 저장되어 폰·PC가 같은 데이터를 공유합니다.
  설정 방법은 [google-sheets-sync/설정방법.md](google-sheets-sync/설정방법.md) 참고.
- 수동 백업/이전은 앱 안에서 `JSON` 내보내기 → 새 기기에서 `불러오기` (운동 기록만 포함 — 체중/식단은
  클라우드 동기화로만 이전됩니다).
...
**체중**, **식단** 탭에서 운동과 별개로 매일 기록할 수 있습니다.

- **체중**: 날짜별 체중(kg/lbs), 체지방률, 메모. "추이 분석" 탭에서 "⚖️ 체중 변화"로 그래프도 볼 수 있습니다.
- **식단**: 하루에 여러 끼니를 카드로 추가(아침/점심/저녁/간식), 끼니마다 음식·칼로리·탄수화물·지방·단백질(g)·메모 기록.

운동 기록과 마찬가지로 로컬에 즉시 저장되고, 클라우드 동기화를 켜두면 같은 Google 시트의
`체중`/`식단` 탭에도 사람이 보기 좋은 표로 자동 반영됩니다.
